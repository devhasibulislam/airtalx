...
function Message() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const result = await axios.get(`${process.env.REACT_APP_BASE_API}/userdata`);
      ...
      if (selectedUser && user) {
        try {
          const response = await axios.get(
            `${process.env.REACT_APP_BASE_API}/message/${user?._id}/${selectedUser?._id}`
          );
  ...
  useEffect(() => {
    const fetchNewMessages = async () => {
      if (user) {
        try {
          const response = await axios.get(
            `${process.env.REACT_APP_BASE_API}/message/new/${user?._id}`
          );
    ...
    if (messageText.trim() !== "" && user && selectedUser) {
      ...
      try {
        const response = await axios.post(
          `${process.env.REACT_APP_BASE_API}/message`,
          newMessage
        );
  ...
  const markAsRead = async (messageId) => {
    try {
      await axios.patch(
        `${process.env.REACT_APP_BASE_API}/message/read/${messageId}`
      );
      setNewMessages(
