...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_BASE_API}/userdata`);
        setData(res.data);
      } catch (error) {
      ...
      try {
        const res = await axios.get(`${process.env.REACT_APP_BASE_API}/postjobs`);
  ...
  const employeer = datas.filter(user => user.role === 'employer');
